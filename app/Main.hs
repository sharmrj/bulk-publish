...
main = do
  [importReportPath, owner, repo, ref, n] <- getArgs
  print "Getting paths from Import Report"
  s <- mkEntrySelector "xl/sharedStrings.xml"
  bs <- withArchive importReportPath (getEntry s)
  let paths = getPaths . parse $ bs
  concurrentlyLimited (read n) (map (\path -> makePOSTCalls [path, owner, repo, ref]) paths)
...
concurrentlyLimited n tasks = conc n (zip [0 ..] tasks) [] []
  where
    ...
    conc n [] ongoing results = conc 0 [] ongoing results
    -- Add a new task to the pool (should a resource be available)
    conc n ((i, x) : xs) ongoing results = do
      t <- async $ (i,) <$> x
      conc (n - 1) xs (t : ongoing) results
  ...
  where
    post f mes = do
      ...
      case resp of
        ...
        Right resp -> handleBody f resp mes
    handleBody f resp mes = do
      ...
      case statusCode of
        ...
        otherwise -> print $ statusCode ++ " Failed to" ++ take ((length mes) - 3) mes ++ " " ++ args !! 0
